...
def train_s2m2(base_loader, base_loader_test, model, start_epoch, stop_epoch, params):
    val_acc_best = 0.0
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        if use_cuda:
            inputs, targets = inputs.cuda(), targets.cuda()
        lam = np.random.beta(args.alpha, args.alpha)
        f , outputs , target_a , target_b = model(inputs, targets, mixup_hidden= True , lam = lam)
        loss = mixup_criterion(criterion, outputs, target_a, target_b, lam)
        train_loss += loss.data.item()
        optimizer.zero_grad()
        loss.backward()
        ...
        targets = Variable(torch.stack(targets_,0)).cuda()
...
if __name__ == '__main__':
    ...
    if params.method =='S2M2_R'
        ...
        model.cuda()
        model = train_s2m2(base_loader, val_loader, model, start_epoch, start_epoch+stop_epoch, params)
